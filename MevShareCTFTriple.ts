...
const sendTestBackrunBundle = async (
    ...
    for (let index = 0; index < 3; index++) {
        const tx: TransactionRequest = {
            ...
            to: log.address,
            nonce: await wallet.getNonce() + index,
            value: 0,
            gasLimit: 1000000,
            data: claimReward,
    ...
    const bundle = [
        {hash: pendingTx.hash},
        {tx: await wallet.signTransaction(txs[0]), canRevert: false},
        {tx: await wallet.signTransaction(txs[1]), canRevert: false},
        {tx: await wallet.signTransaction(txs[2]), canRevert: false},
    ]
...
const handleBackrun = async (
    ...
    if (pendingTx.logs && pendingTx.logs[0].address == "0x1ea6fb65bab1f405f8bdb26d163e6984b9108478"){
